...
The lab environment contains some NGINX instances that are not yet connected to NIM. We will use the ``NIM Scan`` feature
to discover the instances, add them to NIM, and find any CVE's in them.
...
The scan tool needs to know the subnet mask and ports you want to scan.  
UDF uses the subnet ``10.1.1.0/24``.  Change the subnet to this value and leave the ports as ``80,443,8080,8081``.
...
Click on on the “Unmanaged” instance 10.1.1.5 and a slide out dialog box will provide you with instructions on how to install the nginx-agent on the server. 
WAIT! Do not install this yet. 

.. image:: ./UI-scan-install2.png

For module 2, we will focus on managing instances.

Proceed to the next lesson: [Next Lesson(https://github.com/bwolmarans/f5-agility-labs-nginx/blob/develop/docs/class4/module2/2.0-install-agent.rst)]
