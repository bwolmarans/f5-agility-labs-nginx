...
Step 4 - OPTIONAL: Install on Centos 7 
------

NOTE: You can skip this unless you want to understand specifically how to install on Centos 7 or other unsupported operating systems.

TLDR;

Centos 7  went end of life June 2024 and is unsupported by Red Hat and therefore is unsupported by NGINX Instance Manager.
So a normal of Agent will not work.  
However we have two boxes running Centos 7 and NGINX OSS in the lab.
So we would need to do a workaround.
...
Use the ``WEB SHELL`` link and do the following: 

Edit the /etc/nginx-agent/nginx-agent.conf file to have only the following content:
...
And make sure nginx-agent is running.
